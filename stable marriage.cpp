...
/* In this program it checks if a theoretical marriage is stable by cross checking how much the current spouses like eachother vs how much they like 
the other males/females. I used the same main method from 1d queens which matches a new couple, and after having a complete set, they are set to the 
ok function. If they pass the row test which checks that someone isnt married twice, and if they pass the long if statement which checks if their 
marriage isnt stable, that solution is passed to the print function where it prints out the men and womans pairing. */
